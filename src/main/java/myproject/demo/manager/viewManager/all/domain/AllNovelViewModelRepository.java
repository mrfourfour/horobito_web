...
public interface AllNovelViewModelRepository extends JpaRepository<NovelViewModel, Long> {
    ...
    @Query(value = "select semi.*\n" +
            "from (\n" +
            "         select ROW_NUMBER() over (ORDER BY pre.update_time asc ) as updateTimeRaking , pre.*\n" +
            "         from (select * from novel_view_model nvm where nvm.deleted=false order by nvm.update_time asc ) as `pre`) as semi\n" +
